    ...
    <div className="border-t border-line px-4 py-3">
      {loading ? <p className="text-note text-ink-muted">Loading transaction versions…</p> : revisions.length ? <ol className="space-y-4">{revisions.map((revision) => {
        ...
        return <li key={revision.revisionNumber} className="text-note">
          <p className="font-medium text-ink">Version {revision.revisionNumber}<span className="ml-2 font-normal text-ink-muted">{formatInstant(revision.createdAt, timeZone)} · {revision.source === "conversation_edit" ? "Conversation edit" : revision.source === "ledger_edit" ? "Ledger edit" : revision.source === "remove" ? "Removed" : revision.source === "restore" ? "Restored" : revision.source === "account_deleted" ? "Account deleted" : "Created"}</span></p>
          {changes.length ? <ul className="mt-1.5 space-y-1 text-ink-muted">{changes.map(([field, change]) => <li key={field}><span className="font-medium text-ink-body">{revisionFieldLabel(field)}:</span> {revisionValue(field, change.before, currency, timeZone)} → {revisionValue(field, change.after, currency, timeZone)}</li>)}</ul> : <p className="mt-1 text-ink-muted">{revision.source === "account_deleted" ? "Link to the deleted account removed. Transaction details were preserved." : "Initial saved version."}</p>}
        </li>;
