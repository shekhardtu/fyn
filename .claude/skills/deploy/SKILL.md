...
| `backend reported unhealthy` + `RuntimeError: Unsafe authentication configuration` | The app's own fail-closed gate (`backend/app/config.py` `require_production_auth_config`). With `ENVIRONMENT=production` it refuses to boot when `AUTH_SECRET` is missing, weak, or still the development placeholder, or when `OTP_DEBUG_ECHO=true`. Migrations have already run by this point | Fix the unsafe value in `/opt/fyn/backend/.env`, then `./infra/deploy/deploy.sh api` |
| Healthy backend logs `[auth] email sign-in is unavailable` or `[auth] phone sign-in is unavailable` | That OTP channel has no configured delivery provider. The unavailable channel is refused at send time; other configured sign-in methods and the API remain available | Configure Postmark (`POSTMARK_SERVER_TOKEN` + `POSTMARK_FROM_EMAIL`) or MSG91 (`MSG91_AUTH_KEY` + `MSG91_TEMPLATE_ID`) only if that channel should be offered |
